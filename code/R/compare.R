compare <- function(truth,
  ...
  for(param in parameters){
    ...
    if (len==1){
      ...
      table1[[param]]=mcmcmeasures(npops,param,truth[[param]],scenario=scenario,summary.L=summary.L, summary.CL=summary.CL)$table1
      table2[[param]]=mcmcmeasures(npops,param,truth[[param]],scenario=scenario,summary.L=summary.L, summary.CL=summary.CL)$table2
    }else{
      for(i in 1:len){
        #trapeze plots
        plotdata(paste0(param,"[",i,"]"),
                 truth[[param]][i],
                 filename=paste0("output/scenario",scenario,"/results/plot",param,i,".pdf"),
                 xlabel=bquote(hat(italic(~ .(as.name(paste0(param,i))))) ~ "via the true joint likelihood approach"),
                 ylabel=bquote(hat(italic(~ .(as.name(paste0(param,i))))) ~ "via the composite likelihood approach"),
                 summary.L=summary.L,
                 summary.CL=summary.CL)
        
        #boxplots
        pdf(paste0("output/scenario",scenario,"/results/boxplot",param,i,".pdf"))
        par(cex=1.5,mar=c(6.5, 5.5, 2.5, 2))
        boxplot(getstats(paste0(param,"[",i,"]"),"Mean",summary.L=summary.L, summary.CL=summary.CL),
                ylab=bquote(hat(italic(~ .(as.name(paste0(param,i))))) ~""),
                names=c("true joint \n likelihood \n approach","composite \n likelihood \n approach"),
                las=2)
        #ylim=c(0.05,0.5)) #at =c(1,2,3,4, 6,7,8,9, 11,12,13,14)
        abline(h=truth[[param]][i])
        graphics.off()
        
        #Histograms of the difference of the absolute error
        diffabserrorhist(paste0(param,"[",i,"]"),truth[[param]][i],paste0("output/scenario",scenario,"/results/histerror",param,i,".pdf"),summary.L=summary.L, summary.CL=summary.CL)
        table1[[paste0(param,"[",i,"]")]]=mcmcmeasures(npops,paste0(param,"[",i,"]"),truth[[param]][i],scenario=scenario,summary.L=summary.L, summary.CL=summary.CL)$table1
        table2[[paste0(param,"[",i,"]")]]=mcmcmeasures(npops,paste0(param,"[",i,"]"),truth[[param]][i],scenario=scenario,summary.L=summary.L, summary.CL=summary.CL)$table2
        }
    }
     }
