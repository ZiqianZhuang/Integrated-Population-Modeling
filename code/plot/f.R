...
rmse.plot1<-ggplot(data=data,mapping= aes(x=Year,y=RMSE,colour=Method,shape=Method))+
  ...
  scale_color_manual(values = c("#87ceeb", "#ff0000"))+
  ylab("RMSE of f")+guides(color=F,shape=F)

cp.plot1<-ggplot(data=data,mapping= aes(x=Year,y=CP,colour=Method,shape=Method))+
  geom_point(position=position_dodge(width=0.4))+
  coord_cartesian(ylim = c(0.9,1))+
  scale_color_manual(values = c("#87ceeb", "#ff0000"))+
  guides(color=F,shape=F)+ ylab("CP of f")+
  geom_hline(aes(yintercept=0.95), linetype="dashed", color="black", size=0.5)

s1<-grid.arrange(f1.f,rmse.plot1,cp.plot1,nrow=3)

#Scenario2
data<-read.xlsx("f.xlsx",sheetName="S2",header=T)
data<-na.omit(data)
data$estimate<-data$Truevalue+data$Bias
data$LCI<-data$LCI/2
data$estimate[which(data$Converge=="no")]<-NA
...
f2f<-ggplot(data, aes(Year, estimate)) +
  geom_pointrange(
    aes(ymin = estimate-LCI, ymax = estimate+LCI, color = Method,shape=Method),
    position = position_dodge(0.8)
  )+coord_cartesian(ylim = c(0,6))+
...
s2<-grid.arrange(f2.f,rmse.plot2,cp.plot2,nrow=3)

#Scenario 3
data<-read.xlsx("f.xlsx",sheetName="S3",header=T)
data<-na.omit(data)
data$estimate<-data$Truevalue+data$Bias
data$LCI<-data$LCI/2
data$estimate[which(data$Converge=="no")]<-NA
...
f3f<-ggplot(data, aes(Year, estimate)) +
  geom_pointrange(
    aes(ymin = estimate-LCI, ymax = estimate+LCI, color = Method,shape=Method),
...
s3<-grid.arrange(f3.f,rmse.plot3,cp.plot3,nrow=3)

#Scenario 4
data<-read.xlsx("f.xlsx",sheetName="S4",header=T)
data<-na.omit(data)
data$estimate<-data$Truevalue+data$Bias
data$LCI<-data$LCI/2
data$estimate[which(data$Converge=="no")]<-NA
...
f4f<-ggplot(data, aes(Year, estimate)) +
  geom_pointrange(
    aes(ymin = estimate-LCI, ymax = estimate+LCI, color = Method,shape=Method),
    position = position_dodge(0.4)
  )+coord_cartesian(ylim = c(0,6))+
...
s4<-grid.arrange(f4.f,rmse.plot4,cp.plot4,nrow=3)

#Scenario 5-8
data<-read.xlsx("f.xlsx",sheetName="homo",header=T)
data<-na.omit(data)
data$estimate<-data$Truevalue+data$Bias
data$LCI<-data$LCI/2
data$estimate[which(data$Converge=="no")]<-NA
...
f5f<-ggplot(data, aes(Scenario, estimate)) +
  geom_pointrange(
    aes(ymin = estimate-LCI, ymax = estimate+LCI, color = Method,shape=Method),
...
s5<-grid.arrange(f5.f,rmse.plot5,cp.plot5,nrow=3)

##Multiplot
figure.f<-grid.arrange(s1,s2,s3,s4,s5,nrow=1)

##save as PNG
ggsave("Graph of f.png", plot = figure.f, device = "png",
